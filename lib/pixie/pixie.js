(function() {
  ...
  ScratchObj.prototype.fromJSON = function(json) {
    ...
    this.lists = Array.isArray(json.lists) ? json.lists.map(function(json) {
      var list = List.deserialize(json);
      if (json.width != null) {
        list.watcher = ListWatcher.deserialize(this, list, json);
      }
      return list;
    }, this) : [];
    ...
    this.costume = (Math.round(+json.currentCostumeIndex) % this.costumes.length + this.costumes.length) % this.costumes.length || 0;
    return this;
  };
  ...
  function Stage() {
    ...
    this.el = cl('stage', [
      this.canvas = el('canvas', {className: 'Visual-absolute'})
    ]);
  ...
  Stage.prototype.fromJSON = function(json) {
    ...
    if (Array.isArray(json.children)) {
      ...
      json.children.forEach(function(child, i) {
        if (child.objName) {
          this.sprites.push(children[i] = new Sprite().fromJSON(child));
        }
      }, this);
      json.children.forEach(function(child, i) {
        if (child.listName && child.target) {
          var target = this.findObject(child.target);
          children[i] = ListWatcher.deserialize(target, target.findOrCreateLocalList(child.listName), child);
        }
        if (child.cmd) {
          var watcher = Watcher.deserialize(this, child);
          if (watcher) children[i] = watcher;
        }
      }, this);
  ...
  Stage.prototype.bringToFront = function(child) {
    if (child.isSprite) {
      ...
      var i = sprites.indexOf(child);
      if (i !== -1) sprites.splice(i, 1);
      sprites.push(child);
    } else if (child.isWatcher) {
  ...
  Stage.prototype.updatePixelRatio = function(quiet) {
    ...
    this.pixelRatio = pr;
    this.canvas.style.transform = 'scale('+(1/pr)+','+(1/pr)+')';
  ...
  function ListWatcher(target, list, width, height) {
    ...
    this.el = cl('list-watcher', [
      this.elTitle = cl('list-watcher-title'),
  ...
  ListWatcher.deserialize = function(target, list, j) {
    return new ListWatcher(target, list, +j.width || 100, +j.height || 200).fromJSON(j);
  };

  ListWatcher.prototype.fromJSON = function(j) {
    this.moveTo(+j.x || 0, +j.y || 0);
  ...
  ListWatcher.prototype.updateTitle = function() {
    ...
    this.elTitle.textContent = this.target.isStage ? this.list.name : _('{object}: {name}', {object: this.target.name, name: this.list.name});
  };
  ...
  Editor.prototype.toggleSmallStage = function() {
    ...
    this.el.classList.add('small-stage', this.isSmallStage);
  ...
  TopBar.prototype.showAbout = function() { // NS
    ...
    var d = new Dialog(_('About Pixie'), Dialog.content(
      el('article', [
        el('h1', ['Pixie']),
        el('p', ['An unofficial rewrite of ', link('Scratch', 'https://scratch.mit.edu'), ' in JavaScript, created by ', link('Nathan Dinsmore', 'https://github.com/nathan'), '.']),
        el('p', ['Pixie aims to provide an application nearly identical to the current, Flash-based Scratch editor and player using only web technologies. The source code is ', link('available on GitHub', 'https://github.com/nathan/pixie'), '.']),
        cl('ul', 'links', [
          el('li', [link('Issues', 'https://github.com/nathan/pixie/issues')]),
          el('li', [link('Contributors', 'https://github.com/nathan/pixie/graphs/contributors')])
        ])
      ]),
  ...
  SpritePanel.prototype.renameSprite = function() {
    ...
    var newName = this.elSpriteName.value.trim();
  ...
  function SpriteIcon(panel, sprite) {
    ...
    var extent = {width: 68 + 4, height: 51 + 4};

    this.el = cl('sprite-icon', [
      this.elThumbnail = cl('canvas', 'sprite-thumbnail', extent),
      this.elName = cl('sprite-icon-label')
    ]);
    this.elTmp = el('canvas', extent);
  ...
  SpriteIcon.prototype.update = function() {
    ...
    for (var i = 0, l = ps.length; i < l; i++) {
      for (var j = 0; j < l; j++) {
        if (i !== j) {
          this.context.shadowOffsetX = w + ps[i];
          this.context.shadowOffsetY = h + ps[j];
          this.context.shadowColor = 'rgba(0, 0, 0, .1)';
          this.context.drawImage(this.elTmp, -w, -h);
        }
      }
    }
